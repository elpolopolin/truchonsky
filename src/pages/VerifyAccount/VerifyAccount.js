...
const VerifyAccount = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://skilled-obviously-earwig.ngrok-free.app/verifyOTP', {
        userId,
        otp,
      });
  ...
  const handleResendOTP = async () => {
    try {
      const response = await axios.post('https://skilled-obviously-earwig.ngrok-free.app/resendOTP', { // corregir esto logica de que solo el usuario mismo se pueda reenviar los cosos si esta logeado.
        userId,
      });
